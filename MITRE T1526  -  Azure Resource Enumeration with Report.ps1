...
.NOTES
    PATCHED VERSION (v2)  -  Addresses silent-failure issues vs. the original
    parent script. Three changes from the original big script:

      [P1] Invoke-GraphPaged now retries on HTTP 429 with Retry-After backoff,
           and preserves partial results across page failures (previously: any
           mid-stream error broke the loop and returned 0 rows, making the
           report look empty even when the tenant had thousands of objects).

      [P2] New Invoke-GraphPagedWithFallback helper: if a "rich" $select query
           returns 4xx (common when the operator lacks AuditLog.Read.All for
           signInActivity, or Application.Read.All for passwordCredentials on
           SPNs), the helper automatically retries with a minimal $select
           matching the parent script's field set. Sheets populated via the
           fallback path are flagged in the Metadata sheet with DegradedQuery.
           This trades field richness for row count  -  better to get every user
           with 3 columns than zero users with 11.

      [P3] New Test-GraphPermissions pre-flight probe: before any enumeration
           runs, the script tests each Graph scope it will need and prints a
           clear per-scope PASS/FAIL table. Operator sees up front which data
           will degrade, rather than discovering empty tabs after the run.
#>
...
function Invoke-GraphPaged {
    ...
    while ($next) {
        try {
            ...
            # [P1] Handle 429 with exponential backoff; preserve partial results on other errors
            $status = $null
            try { $status = [int]$_.Exception.Response.StatusCode } catch { }

            if ($status -eq 429 -and $retries -lt $MaxRetries) {
                $retries++
                $retryAfter = 0
                try { $retryAfter = [int]($_.Exception.Response.Headers['Retry-After']) } catch { }
                if (-not $retryAfter -or $retryAfter -le 0) { $retryAfter = [int][Math]::Pow(2, $retries) }
                Write-Host "    [~] 429 throttle on Graph, retry $retries/$MaxRetries after $retryAfter s..." -ForegroundColor DarkYellow
                Start-Sleep -Seconds $retryAfter
                continue   # retry same $next URI
            }

            # [P1] Non-429 or retries exhausted: log and return what we have rather than losing everything.
            # The old behavior was `break` which silently discarded prior pages AND didn't tell the operator
            # the result was partial.
            Write-Host "[-] Graph failed at page $($pagesFetched + 1): $next" -ForegroundColor Red
            Write-Host "    Status: $status  Message: $($_.Exception.Message)" -ForegroundColor Red
            Write-Host "    Returning $($results.Count) partial row(s) collected so far." -ForegroundColor Yellow
            # Record the partial state globally so the Metadata sheet can flag it.
            if (-not $script:PartialQueries) { $script:PartialQueries = New-Object System.Collections.Generic.List[object] }
            $null = $script:PartialQueries.Add([PSCustomObject]@{
                Uri           = $Uri
                PagesFetched  = $pagesFetched
                RowsCollected = $results.Count
...
# -----------------------------------------------------------------------------
# [P2] Invoke-GraphPagedWithFallback
#
# Some Graph $select fields require elevated scopes or premium licenses:
#   - users?$select=signInActivity          requires AuditLog.Read.All + Entra P1/P2
#   - servicePrincipals?$select=passwordCredentials,keyCredentials
#                                           requires Application.Read.All
#   - applications?$select=passwordCredentials,keyCredentials
#                                           requires Application.Read.All
#
# If the operator lacks the scope / license, the ENTIRE $select query 4xx's
# and we get back zero rows. The small parent script never hit this because it
# asked for 3 vanilla fields only.
#
# This wrapper tries the rich query first, and on 400/401/403 falls back to a
# minimal $select (matching the parent script's field set). Degraded sheets
# are recorded in $script:DegradedQueries so the Metadata / Executive Summary
# can surface the downgrade to the reviewer.
# -----------------------------------------------------------------------------
function Invoke-GraphPagedWithFallback {
    param(
        [Parameter(Mandatory=$true)][string]$RichUri,
        [Parameter(Mandatory=$true)][string]$MinimalUri,
        [Parameter(Mandatory=$true)][string]$Label
    )
    try {
        $rows = Invoke-GraphPaged -Uri $RichUri
        # If we got 0 rows AND the pagination logged a 4xx failure for this URI,
        # fall back rather than trust the empty result.
        $partialHit = $null
        if ($script:PartialQueries) {
            $partialHit = $script:PartialQueries | Where-Object {
                $_.Uri -eq $RichUri -and $_.FailureStatus -ge 400 -and $_.FailureStatus -lt 500
            } | Select-Object -First 1
        }
        if ($rows.Count -eq 0 -and $partialHit) {
            Write-Host "    [!] $Label query returned 0 rows with $($partialHit.FailureStatus). Falling back to minimal fields..." -ForegroundColor Yellow
            $rows = Invoke-GraphPaged -Uri $MinimalUri
            if (-not $script:DegradedQueries) { $script:DegradedQueries = New-Object System.Collections.Generic.List[object] }
            $null = $script:DegradedQueries.Add([PSCustomObject]@{
                Label       = $Label
                OriginalUri = $RichUri
                FallbackUri = $MinimalUri
                Reason      = "HTTP $($partialHit.FailureStatus) on rich query"
                RowsAfter   = $rows.Count
            })
            Write-Host "    [+] Fallback returned $($rows.Count) rows (reduced field set)." -ForegroundColor Green
        }
        return ,$rows
    } catch {
        Write-Host "    [!] $Label rich query failed outright: $($_.Exception.Message)" -ForegroundColor Yellow
        Write-Host "    [!] Trying minimal-fields fallback..." -ForegroundColor Yellow
        try {
            $rows = Invoke-GraphPaged -Uri $MinimalUri
            if (-not $script:DegradedQueries) { $script:DegradedQueries = New-Object System.Collections.Generic.List[object] }
            $null = $script:DegradedQueries.Add([PSCustomObject]@{
                ...
                OriginalUri = $RichUri
                FallbackUri = $MinimalUri
                Reason      = "Exception on rich query: $($_.Exception.Message)"
                RowsAfter   = $rows.Count
            })
            return ,$rows
        } catch {
            Write-Host "    [-] $Label fallback ALSO failed: $($_.Exception.Message)" -ForegroundColor Red
...
# -----------------------------------------------------------------------------
# [P3] Test-GraphPermissions
#
# Pre-flight probe against a handful of Graph endpoints that correspond to the
# scopes this script needs for full fidelity. Prints a PASS/FAIL table so the
# operator sees  -  before any enumeration  -  which sheets will degrade.
#
# This runs ONCE at startup. It does not try every endpoint the script uses;
# it just samples the "gatekeeper" URIs for each scope class.
# -----------------------------------------------------------------------------
function Test-GraphPermissions {
    Write-Host "`n[*] Pre-flight: probing Graph scopes available on this token..." -ForegroundColor Cyan

    $probes = @(
        @{ Scope='Directory.Read.All (baseline)';       Uri='https://graph.microsoft.com/v1.0/users?$top=1'; Affects='Users, Groups, basic directory' }
        @{ Scope='AuditLog.Read.All';                    Uri='https://graph.microsoft.com/v1.0/auditLogs/signIns?$top=1'; Affects='signInActivity on Users, StaleUsers sheet' }
        @{ Scope='Reports.Read.All';                     Uri='https://graph.microsoft.com/v1.0/reports/authenticationMethods/userRegistrationDetails?$top=1'; Affects='MFAStatus sheet' }
        @{ Scope='Application.Read.All';                 Uri='https://graph.microsoft.com/v1.0/applications?$top=1&$select=passwordCredentials'; Affects='AppCredentials, SPN creds, AppFederatedCredentials' }
        @{ Scope='RoleManagement.Read.Directory';        Uri='https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments?$top=1'; Affects='DirectoryRoleAssignments, PrivilegedGroups' }
        @{ Scope='RoleManagement.Read.Directory (PIM)';  Uri='https://graph.microsoft.com/v1.0/roleManagement/directory/roleEligibilityScheduleInstances?$top=1'; Affects='PIMEligibleRoles sheet' }
        @{ Scope='Policy.Read.All';                      Uri='https://graph.microsoft.com/v1.0/policies/conditionalAccessPolicies?$top=1'; Affects='ConditionalAccess, NamedLocations, CrossTenantAccess' }
        @{ Scope='Policy.Read.All (auth methods)';       Uri='https://graph.microsoft.com/v1.0/policies/authenticationMethodsPolicy'; Affects='AuthMethodsPolicies' }
        @{ Scope='Domain.Read.All';                      Uri='https://graph.microsoft.com/v1.0/domains'; Affects='FederationSettings, external attack surface seeds' }
        @{ Scope='AdministrativeUnit.Read.All';          Uri='https://graph.microsoft.com/v1.0/directory/administrativeUnits?$top=1'; Affects='AdministrativeUnits sheet' }
    )
    ...
    foreach ($p in $probes) {
        ...
        $err  = ''
        try {
            $null = Invoke-RestMethod -Uri $p.Uri -Headers (Get-GraphToken) -ErrorAction Stop
            $pass = $true
        } catch {
            try { $status = [int]$_.Exception.Response.StatusCode } catch { $status = 0 }
            $err = "HTTP $status - $($_.Exception.Message -replace '[\r\n]+',' ' | Select-Object -First 1)"
    ...
    Write-Host ""
    Write-Host "    Scope probe results:" -ForegroundColor Cyan
    foreach ($r in $results) {
        $color = if ($r.Status -eq 'PASS') { 'Green' } else { 'Red' }
        Write-Host ("      [{0}] {1,-42} -> {2}" -f $r.Status, $r.Scope, $r.Affects) -ForegroundColor $color
        if ($r.Status -eq 'FAIL' -and $r.Error) {
    ...
    if ($failed -gt 0) {
        Write-Host ""
        Write-Host "    [!] $failed of $($results.Count) scopes FAILED. Affected sheets will fall back to minimal field sets or be empty." -ForegroundColor Yellow
        Write-Host "    [!] To get full fidelity, grant the listed scopes to the account you authenticated with." -ForegroundColor Yellow
        $continue = Read-Host "`n    Continue anyway? (yes/no)"
        if ($continue -ne 'yes') {
            Write-Host "[!] Aborted by operator after permission probe." -ForegroundColor Yellow
            Disconnect-AzAccount | Out-Null
            exit
        }
    } else {
        Write-Host ""
        Write-Host "    [+] All probed scopes available. Proceeding at full fidelity." -ForegroundColor Green
    }

    # Save to script scope so Metadata sheet can embed the results.
    $script:ScopeProbeResults = $results
}
...
function Enumerate-Users {
    ...
    # [P2] signInActivity requires AuditLog.Read.All + Entra P1/P2. If either is
    # missing, the full $select 403s and we get 0 users. Fallback asks for just
    # the vanilla fields so we at least get the population count and UPNs.
    $richUri    = "https://graph.microsoft.com/v1.0/users?`$top=999&`$select=id,displayName,userPrincipalName,userType,accountEnabled,mail,jobTitle,department,createdDateTime,onPremisesSyncEnabled,signInActivity"
    $minimalUri = "https://graph.microsoft.com/v1.0/users?`$top=999&`$select=id,displayName,userPrincipalName,userType,accountEnabled,mail,createdDateTime"
    $allUsers   = Invoke-GraphPagedWithFallback -RichUri $richUri -MinimalUri $minimalUri -Label 'Users'
...
function Enumerate-ServicePrincipals {
    ...
    # Service Principals
    # [P2] passwordCredentials / keyCredentials in $select require Application.Read.All.
    # A standard Directory.Read.All-only account 403s on this query and we get 0 SPs.
    # Fallback drops the credential fields; the AppCredentials sheet will be empty from
    # the SP side, but we still enumerate every SPN and the credentials will be picked
    # up from the /applications endpoint if Application.Read.All is present there.
    $richUri    = "https://graph.microsoft.com/v1.0/servicePrincipals?`$top=999&`$select=id,appId,displayName,servicePrincipalType,accountEnabled,appOwnerOrganizationId,publisherName,signInAudience,tags,passwordCredentials,keyCredentials"
    $minimalUri = "https://graph.microsoft.com/v1.0/servicePrincipals?`$top=999&`$select=id,appId,displayName,servicePrincipalType,accountEnabled,appOwnerOrganizationId,publisherName,signInAudience,tags"
    $allSps     = Invoke-GraphPagedWithFallback -RichUri $richUri -MinimalUri $minimalUri -Label 'ServicePrincipals'
    ...
    # App Registrations
    # [P2] Same credential-field 403 pattern as /servicePrincipals. Fall back to
    # non-credential fields if Application.Read.All is not granted. Credential
    # enumeration (the AppCredentials sheet) will be empty in that case  -  which
    # accurately reflects reality, rather than silently reporting "0 apps".
    $richUri    = "https://graph.microsoft.com/v1.0/applications?`$top=999&`$select=id,appId,displayName,signInAudience,createdDateTime,publisherDomain,passwordCredentials,keyCredentials,web,requiredResourceAccess"
    $minimalUri = "https://graph.microsoft.com/v1.0/applications?`$top=999&`$select=id,appId,displayName,signInAudience,createdDateTime,publisherDomain,web"
    $allApps    = Invoke-GraphPagedWithFallback -RichUri $richUri -MinimalUri $minimalUri -Label 'AppRegistrations'
...
# [P3] Run the scope probe before any enumeration starts. This prints a clear
# PASS/FAIL table for every Graph scope the script cares about, and gives the
# operator a chance to bail out if too many scopes are missing. Without this,
# the operator would only discover missing scopes after the run, as empty or
# partial worksheets in the final .xlsx.
...
$Metadata = @(
    ...
    [PSCustomObject]@{ Field='ScriptVersion';         Value='PATCHED v2 (P1 pagination retry, P2 $select fallback, P3 scope probe)' }
)
...
# [P1/P2/P3] Fidelity flags  -  the whole point of the patches is to make these visible.
if ($script:DegradedQueries -and $script:DegradedQueries.Count -gt 0) {
    $Metadata += [PSCustomObject]@{ Field='DegradedQueryCount'; Value=$script:DegradedQueries.Count }
    foreach ($dq in $script:DegradedQueries) {
        $Metadata += [PSCustomObject]@{
            Field = "Degraded_$($dq.Label)"
            Value = "$($dq.Reason) | rows after fallback: $($dq.RowsAfter)"
        }
    }
}
if ($script:PartialQueries -and $script:PartialQueries.Count -gt 0) {
    $Metadata += [PSCustomObject]@{ Field='PartialQueryCount'; Value=$script:PartialQueries.Count }
    foreach ($pq in $script:PartialQueries) {
        $Metadata += [PSCustomObject]@{
            Field = "Partial_$($pq.Uri.Substring([Math]::Max(0,$pq.Uri.Length-80)))"
            Value = "HTTP $($pq.FailureStatus) after $($pq.PagesFetched) page(s), $($pq.RowsCollected) rows captured"
        }
    }
}
if ($script:ScopeProbeResults) {
    $failedScopes = @($script:ScopeProbeResults | Where-Object { $_.Status -eq 'FAIL' })
    $Metadata += [PSCustomObject]@{ Field='ScopesProbed';   Value=$script:ScopeProbeResults.Count }
    $Metadata += [PSCustomObject]@{ Field='ScopesFailed';   Value=$failedScopes.Count }
    foreach ($sp in $failedScopes) {
        $Metadata += [PSCustomObject]@{ Field="MissingScope_$($sp.Scope -replace '[^a-zA-Z0-9]','_')"; Value=$sp.Affects }
    }
}
...
# [P1/P2/P3] Fidelity summary  -  flag any silent data loss loud and early so
# the operator doesn't mistake a permission gap for "the tenant has nothing."
$hasFidelityIssues = ($script:DegradedQueries -and $script:DegradedQueries.Count -gt 0) -or
                     ($script:PartialQueries   -and $script:PartialQueries.Count   -gt 0) -or
                     ($script:ScopeProbeResults -and (@($script:ScopeProbeResults | Where-Object { $_.Status -eq 'FAIL' }).Count -gt 0))
if ($hasFidelityIssues) {
    Write-Host "[!] FIDELITY WARNINGS  -  the following reduced result fidelity:" -ForegroundColor Yellow
    if ($script:ScopeProbeResults) {
        $failed = @($script:ScopeProbeResults | Where-Object { $_.Status -eq 'FAIL' })
        if ($failed.Count -gt 0) {
            Write-Host "    Missing Graph scopes ($($failed.Count)):" -ForegroundColor Yellow
            foreach ($f in $failed) {
                Write-Host ("      - {0,-42} affects: {1}" -f $f.Scope, $f.Affects) -ForegroundColor DarkYellow
            }
        }
    }
    if ($script:DegradedQueries -and $script:DegradedQueries.Count -gt 0) {
        Write-Host "    Queries that fell back to minimal `$select ($($script:DegradedQueries.Count)):" -ForegroundColor Yellow
        foreach ($dq in $script:DegradedQueries) {
            Write-Host ("      - {0,-24} -> {1} rows (reason: {2})" -f $dq.Label, $dq.RowsAfter, $dq.Reason) -ForegroundColor DarkYellow
        }
    }
    if ($script:PartialQueries -and $script:PartialQueries.Count -gt 0) {
        Write-Host "    Paginated queries that returned partial results ($($script:PartialQueries.Count)):" -ForegroundColor Yellow
        foreach ($pq in $script:PartialQueries) {
            Write-Host ("      - HTTP {0} after page {1} -> {2} rows captured" -f $pq.FailureStatus, $pq.PagesFetched, $pq.RowsCollected) -ForegroundColor DarkYellow
        }
    }
    Write-Host "    If counts look low, this is usually WHY. Grant the listed scopes and re-run." -ForegroundColor Yellow
    Write-Host ""
}
